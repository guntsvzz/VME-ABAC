...
%%Using theta1-theta6 fromQ8 then use FK to plot%%
A1 = link([0 0 Data_rad(1,1) L1,0]);
A2 = link([pi/2 0 pi/2 d2,1]);
A3 = link([0 0 0 d3L2,1]); %where d3L2 = L2+d3
A4 = link([-pi/2 0 Data_rad(1,4) 0,0]);
A5 = link([pi/2 0 Data_rad(1,5) 0,0]);
L6 = link([0 0 Data_rad(1,6) L6,0]);
RRRandWrist = robot({A1 A2 A3 A4 A5 L6});
